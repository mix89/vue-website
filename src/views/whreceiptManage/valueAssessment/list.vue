      ...
      <div class="page_title_main">
        ...
        <div class="page_main_cont">
            ...
            <div class="fix">
              <div style="float:right;margin-right:15px;">
                <div class="search_box">
                  ...
                  <el-form-item label="评估价值" class="money-item" v-if="this.statusItem == '20'">
                    <div class="money-box">
                      <el-input v-model="startCostAmount" placeholder="请输入金额"></el-input>
                    </div>
                    <span>-</span>
                    <div class="money-box">
                      <el-input v-model="endCostAmount" placeholder="请输入金额"></el-input>
                    </div>
                  </el-form-item>

                  <el-form-item label="" class="datepicker-item2" style="width: 328px">
                    <div class="datepicker-box" style="width: 328px">
                      <el-select v-model="selectTime" size="small" style="width: 120px" :disabled="this.statusItem != '20'" slot="prepend" class="prepend">
                        <el-option label="评估申请时间" value="1"></el-option>
                        <el-option label="评估时间" value="2"></el-option>
                      </el-select>
                      <el-date-picker
                        v-model="dateTime"
                        type="daterange"
          ...
          >
            ...
            <el-table-column label="评估价值(元)" width="162" v-if="this.statusItem == '20'">
              <template slot-scope="scope">{{scope.row.costAmount | currency('') }}</template>
            </el-table-column>
            <el-table-column prop="createtime" label="评估申请时间" width="110">
              ...
              <!-- formartDateTimeMinute('') -->
            </el-table-column>
            <el-table-column prop="createName" label="评估申请人" width="110">
              <template slot-scope="scope">
                <div>{{scope.row.createName}}</div>
                <!-- <div v-if="scope.row.processDetailCreateNickName">
                  <span>{{scope.row.processDetailCreateNickName}}</span>
                </div>
                <div v-else>
                  <span>{{scope.row.processDetailCreateName}}</span>
                </div> -->
              </template>
            </el-table-column>
            <el-table-column prop="status" label="状态" width="110">
              <template slot-scope="scope">
                    <span v-if="4 == scope.row.status">待评估</span>
                    <span v-else>已评估</span>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="100" v-if="this.statusItem != '20'">
              <template slot-scope="scope">
                <div v-if="scope.row.status == 4">
                  <el-button
                    @click="toValueAssess(scope.row)"
...
export default {
  ...
  methods: {
    ...
    handleSelect(key) {
      this.statusItem = key
      this.startCostAmount = null
      this.endCostAmount = null
    ...
    init() {
      ...
      let processDetailCreateName = null;
      if (this.select === "1" && this.searchData != "") {
        sysWhrNo = this.searchData;
      } else if (this.select === "2" && this.searchData != "") {
        pledgorName = this.searchData;
      } else if (this.select === "3" && this.searchData != "") {
        wName = this.searchData;
      } else if (this.select === "4" && this.searchData != "") {
      ...
      let data = {
        ...
        startCostAmount:
          Number(this.startCostAmount) == null || Number(this.startCostAmount) == 0
            ? null
            : area(Number(this.startCostAmount)),
        endCostAmount:
          Number(this.endCostAmount) == null || Number(this.endCostAmount) == 0
            ? null
            : area(Number(this.endCostAmount)),
        ...
        processDetailCreateName: processDetailCreateName
      };
      requestPost("/api/auth/site/whreceipt/value/list", data).then(
        response => {
