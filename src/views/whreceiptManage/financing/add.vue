        ...
        <div class="page_head_container">
          <div class="page_container">
            <div class="page_head_menu">
              <Logo/>
              <div class="page_head_title">新 增 仓 单</div>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    singleDialogue2(index) {
      ...
      //  return this.ruleForm2.whreceiptCommodityList.splice(index, 1);
    },
      ...
      Promise.all([p1, p2])
        .then(function() {
          ...
          newFileList.map((item, index) => {
            return (_that.ruleForm2.whreceiptCommodityList[
              index
            ].whreceiptAttachment = item);
          });
          ...
          //新增仓单操作
          requestPost("/api/auth/site/whreceipt/add", addData).then(
            response => {
              if (response.data.code === 0) {
                _that.$message.success("新增仓单成功");
                _that.$router.go(-1);
    ...
    deleteCargo(index) {
      ...
      // let _index = index
      // let _that = this
      // var index = this.ruleForm.whreceiptCommodityList.indexof(item);
      // if(index!==-1){
      ...
      //     alert(_index)
      //     alert(_that)
      //       _that.ruleForm.whreceiptCommodityList.splice(_index, 1);
      //   })
      //   .catch(() => {
      //      alert(2)
      //   });
