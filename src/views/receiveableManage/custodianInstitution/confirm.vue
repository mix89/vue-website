  ...
  <div>
      ...
      <div class="order_number_show mt10">
        <div class="fix">
          系统单号：{{ruleForm.sysReceNo}}
              <span class="waiting_txt">平台待确认</span>
    ...
    <!-- 弹出框 -->
            <div class="submit_buttons" style="background:#fff;">
                <el-button type="primary" @click="submitForm('ruleForm4')" class="primary_btn">确认</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      rules4: {
        confirm: [
            { required: true, message: '请选择是否确认已经接受仓单凭证', trigger: 'change' }
        ],
  ...
  created() {
    ...
    requestPost("/api/auth/site/receivable/storage/addInfoShow", {
  ...
  methods: {
      ...
      this.$refs[formName].validate(valid => {
        ...
        // #22
          ...
          let confirmData = {};
          confirmData.receId = this.ruleForm.receId;
          confirmData.keepingResult=this.ruleForm4.confirm
          confirmData.remark = this.ruleForm4.remark
          confirmData.attachment = this.ruleForm4.attachment
          confirmData.attaName = this.ruleForm4.attaName
        //     "whrId": 27,
      	// " keepingResult ":8,
        // "handleExplain":"可以准予通过",
          ...
          requestPost("/api/auth/site/receivable/keeping", confirmData)
            .then(response => {
              if (response.data.code === 0) {
