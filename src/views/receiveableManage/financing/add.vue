        ...
        <div class="page_head_container">
          <div class="page_container">
            <div class="page_head_menu">
              <Logo/>
              <div class="page_head_title">新 增 应 收 单</div>
            </div>
          </div>
        </div>
      </div>
                ...
                <div class="infoshow_cell_body minh60">
                    <!-- #11 -->
                     <!-- <div v-for="(item,index) in  filelist2"  :key='index'>
                                              <el-row>
                                                <el-col :span="8">
                                                   <el-button class="showPicBtn"  type="text" @click="downloadFile(item.fileName,item.attachment)"  >{{item.attachment}}</el-button>

                                                </el-col>
                                                <el-col :span="16">
                                                   <span>{{item.createTime}}</span>
                                              <el-button type="text"  @click.prevent="downloadFile(item.fileName,item.attachment)">下载</el-button>
                                           <el-button type="text" @click="changeNameDialog(filelist2,item)" >编辑</el-button>
                                           <el-button type="text" @click="removeFileFn(item,filelist2)" >删除</el-button>
                                                </el-col>
                                              </el-row>
                                           </div> -->
      ...
      <div class="infoshow_box--cargo">
        ...
        <el-form>
          <el-form-item class="submit_buttons">
            ...
            <el-button @click="cancel({form1:ruleForm,form2:ruleForm2})" class="cancel_btn">取消</el-button>
          </el-form-item>
        </el-form>
      </div>
      <!-- 弹出框 -->
...
import {validateReNum,validateBuyName,validateSellName,validateGoodsType,validateGoodsPrice,validateGoodsNumber} from "@/utils/validateRules"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      cargoUnitList: [{ key: 0, value: "米（m）" }, { key: 1, value: "厘米(cm)" },{ key: 2, value: "毫米（mm）" },{ key: 3, value: "千克（kg）" },{ key: 4, value: "克（g）" }],
      //上传
      uploadUrl: uploadUrl,
      //弹出框
      ...
      // rules2: {},
      rules: {
        dateTime:[{type:'date',required:true,message:'请选择时间',trigger:'change'}],
        keepingId: [
        ...
        receNo: [
          { validator: validateReNum, trigger: ['blur','change']}
        ],
        sellName: [
           { validator: validateSellName, trigger: ['blur','change']}
        ],
        buyName:[
          {validator: validateBuyName, trigger: ['blur','change'] }
        ],
       ...
       rules2: {
        ...
        // 货物名称
        name: [
          { required: true, message: "请输入正确的货物名称", trigger: ['blur','change']},
          { min: 1, max: 40, message: "长度为40字符以内", trigger: ['blur','change']}
        ],
        // 货物规格
        type:[
          { validator: validateGoodsType, trigger: ['blur','change'] }
        ],
        category:[
        ...
        price:[
          { validator: validateGoodsPrice, trigger: ['blur','change']}
        ],
        unit:[
             { required: true, message: '请选择单位', trigger: 'change' }
        ],
        count:[
         {  validator:validateGoodsNumber,  trigger: ['blur','change']}
        ],
    ...
    requestPost("/api/auth/site/customer/list", { customerType: 2 }).then(
      response => {
        ...
        if (response.data.code === 0) {
          ...
          console.log(this.form.keeping.list);
        }
      }
    );
  },
  ...
  methods: {
      ...
      Promise.all([p1, p2])
        .then(function() {
          ...
          newFileList.map((item, index) => {
            return (_that.ruleForm2.receivableCommodityList[index].receivableAttachment = item);
          });
    ...
    cancel(formData) {
             ...
             let testArr=[]
                  ...
                  for(var item in formData.form1){   //质押信息       //遍历对象的值，判断是否是空值
                        if(item !="pledgorId" && item !="totalAmount" && item!='wfProcessDetail' && item!='receType' && formData.form1[item]!=''){  //排除这几个值，因为默认有值
                        testArr.push(formData.form1[item])
                    }
                  }
                    
              let testArr1=[]
                 
                      formData.form2.receivableCommodityList.forEach((item)=>{
                                 for(var el in item){
                                  if(el!='isMark' && el!='totalPrice' && item[el]!=''){  //获取信息，这里没有判断文件上传是否为空
                                         testArr1.push(item[el])
                                     }
                               }
                                                            })
                           if(testArr.length==0 && testArr1.length==0 ){
                          this.$router.go(-1)
                      }else if(testArr.length!=0 || testArr1.length!=0){
