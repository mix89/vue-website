         ...
         <template slot="empty">
          ...
          <el-table-column prop="sysWhrNo" label="系统单号" width="178">
            <template slot-scope="scope">
                 ...
                 <el-button @click="toShow(scope.row)" type="text" size="small" class="info_txt--number">{{scope.row.sysWhrNo}}</el-button>
              </div>
              <div v-else>
                 <el-button @click="toConfirm(scope.row)" type="text" size="small" class="info_txt--number">{{scope.row.sysWhrNo}}</el-button>
              </div>
          ...
          <el-table-column prop="createName" label="申请人" width="110">
            <template slot-scope="scope">{{scope.row.processDetailCreateNickName}}
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    toConfirm(row){
        this.$router.push({name:'custodianInstitutionConfirm',query: { whrId: row.whrId }})
    },
  ...
  tableCellStyle(){
    ...
    toShow(row) {
      this.$router.push({
        path: "custodianInstitutionShow",
        query: { whrId: row.whrId }
      });
    },
    toEdit(row){
      this.$router.push({name:'whreceiptBasicsManageEdit', query: {whrId: row.whrId}})
    },
    toCheck(row){
      this.$router.push({name:'whreceiptBasicsManageCheck', query: {whrId: row.whrId}})
    },
    toVoucher(row){
    ...
    //   api/site/whreceipt/value/list

      //#r
      requestPost("/api/auth/site/whreceipt/storage/list",data).then(response => {
