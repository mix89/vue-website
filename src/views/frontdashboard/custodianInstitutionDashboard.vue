...
import custodianList from "@/views/frontdashboard/custodianInstitution/custodianList"
...
export default {
  data() {
    ...
    this.ringColors = ["#00ACF3", "#7543F2","#DBDBDB"];
  ...
  components: { VeRing,infoList,custodianList },
  created() {
    // #3
        

       //企业管理账号首页制单个数数据统计
       requestPost("/api/auth/site/thirdParty/index",{customerType:2}).then(response => {
    ...
    // 环状图
    requestPost("/api/auth/site/thirdParty/distribution",{customerType:2}).then(response => {
      if (response.data.code === 0) {
