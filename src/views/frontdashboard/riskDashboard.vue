...
import riskList from "@/views/frontdashboard/risk/riskList"
...
export default {
  ...
  components: { VeRing,infoList,riskList },
  created() {
    // #3
        

       //企业管理账号首页制单个数数据统计
       requestPost("/api/auth/site/thirdParty/index",{customerType:4}).then(response => {
    ...
    // 环状图
    requestPost("/api/auth/site/thirdParty/distribution",{customerType:4}).then(response => {
      if (response.data.code === 0) {
