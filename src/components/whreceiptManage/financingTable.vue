        ...
        <template slot="empty">
          ...
          <el-table-column prop="sysWhrNo" label="系统单号" width="178">
            <template slot-scope="scope">
              <el-button @click="toShow(scope.row)" type="text" size="small" class="info_txt" style="font-size:12px;">{{scope.row.sysWhrNo}}</el-button>
            </template>
          </el-table-column>
           ...
           <el-table-column label="操作" width="80">
            <template slot-scope="scope">
               <div v-if="scope.row.status == '1'">
                  <el-button @click="toCheck(scope.row)" type="text" size="small"  class="waiting_txt">审核</el-button>
              </div>
              <div v-else-if="scope.row.status=='8'">
                   <el-button @click="toApply(scope.row)" type="text" size="small"  class="info_txt">申请融资</el-button>
              </div>
              <div v-else-if="scope.row.status =='1' && user.userRoles[0].roleCode == 'L1001'">
                 <el-button @click="toEdit(scope.row)" type="text" size="small"  class="info_txt">编辑</el-button>
              </div>
               <div v-else>
                <div>
...
// { key: 2, value: "订单时间" },
  ...
  tableCellClass(){
    ...
    toApply(row){
      this.$router.push({name:'whreceiptBasicsManageApply', query: {whrId: row.whrId}})
    },
    // toVoucher(row){
  ...
  watch: {
    ...
    //     this.startTime = formartDate(dateTime[0]) + " 0:0:0"
    //   } else {
    //     this.startTime = null;
    //     this.endTime = null;
    //   }
